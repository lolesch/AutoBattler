...
namespace Code.Runtime.Pawns
{
    ...
    public sealed class PawnCombatController : IPawnCombatController
    {
        ...
        public PawnCombatController(IPawn pawn, IHexGrid hexGrid = null)
        {
            _pawn       = pawn;
        ...
        public void SetHexContext(IHexGrid grid) => _hexGrid = grid;
        ...
        private (List<IPawn> targets, List<Hex> hexes) ResolvePayloadTargets(PayloadBehavior behavior)
        {
            ...
            switch (behavior.Targeting)
            {
                ...
                case PayloadTargeting.Self:
                    return (new List<IPawn> { _pawn }, new List<Hex> { ((IHexOccupant) _pawn).HexPosition });

                case PayloadTargeting.Aoe:
                    if (_hexGrid == null) { LogMissingContext(); return FallbackToSingleTarget(); }
                    return (
                        (_hexGrid.GetOccupantsInRange( ((IHexOccupant) _target).HexPosition, behavior.Range, PawnTeam.Enemy) as List<IPawn>).ToList(),
                        ((IHexOccupant) _target).HexPosition.HexRange(behavior.Range)
                    );

                case PayloadTargeting.Line:
                    if (_hexGrid == null) { LogMissingContext(); return FallbackToSingleTarget(); }
                    var lineHexes = ((IHexOccupant) _pawn).HexPosition.HexLine(((IHexOccupant) _target).HexPosition);
                    var linePawns = new List<IPawn>();
                    foreach (var hex in lineHexes)
                        foreach (var p in _hexGrid.GetOccupantsInRange(hex, 0, PawnTeam.Enemy))
                            if( p is IPawn pawn && !linePawns.Contains(pawn)) linePawns.Add(pawn);
        ...
        private (List<IPawn>, List<Hex>) FallbackToSingleTarget() =>
            _target == null
                ...
                : (new List<IPawn> { _target }, new List<Hex> { ((IHexOccupant) _target).HexPosition });
            ...
            foreach (var target in targets)
            {
                switch (effect.EffectType)
                {
                    ...
                    case PositionEffectType.Pull:
                        ApplyDisplacement((IHexOccupant)target, effect);
                        break;
        ...
        /// </summary>
        private void ApplyDisplacement(IHexOccupant target, PositionPayloadEffect effect)
        {
            var line = ((IHexOccupant)_pawn).HexPosition.HexLine(target.HexPosition);
            ...
            _target.MoveTo(target.HexPosition.Add(step.Scale(effect.Distance)));
        }

        private void ApplyTerrainEffect(TerrainPayloadEffect effect, List<Hex> hexes)
        {
            if (_hexGrid == null) { LogMissingContext(); return; }
    ...
    public interface IPawnCombatController
    {
        ...
        void SetHexContext(IHexGrid grid);
        void StartCombat();
        void StopCombat();
    }
}